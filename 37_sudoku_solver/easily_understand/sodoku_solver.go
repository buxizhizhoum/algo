...
func solveSudoku(board [][]byte)  {
	m := len(board)
	n := len(board[0])
	// 通过i, j不断往后走，每一次都在所有的数字之中进行尝试
	backtrack(board, 0, 0, m, n)
}

func backtrack(board [][]byte, i, j int, m, n int) bool {
	// 已经走出了范围，表示找到了一个解
	if i > m-1 {
		return true
	}
	// 已经有数字，直接看后面的结果
	if board[i][j] != '.' {
		newX, newY := calculateNextCoordinate(i, j+1, m, n)
		return backtrack(board, newX, newY, m, n)
	}
	// 需要填数字，在所有的数字之间尝试
	for number:=byte('1');number<=byte('9');number++ {
		...
		newX, newY := calculateNextCoordinate(i, j+1, m, n)
		// 找到一个就返回
		res := backtrack(board, newX, newY, m, n)
		if res {
			return true
		}
		// 如果尝试不行，回退操作
